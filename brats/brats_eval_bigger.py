...
def compare(start,step,TP,P,T, name_list):
    ...
    for obj in os.listdir(gt_folder): 
        ...
        full_path34 = predict_folder34 + pred_name + '.npy'
        full_path50 = predict_folder50 + pred_name  +'.npy'
        
        if os.path.isfile(predict_folder34 + pred_name + '.npy'):
                ...
                array34_sum = array34.sum()
                array50_sum = array50.sum()

                if array50_sum > array34_sum:
                      predict = array50
                else:
                      predict = array34    
...
def do_python_eval(predict_folder, gt_folder, name_list, num_cls=2):
    TP = []
    P = []
    T = []
    for i in range(num_cls):
